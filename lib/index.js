  ...
  const scrapeThreadFromHtml = /* @__PURE__ */ __name(async (threadId) => {
    ...
    try {
      ...
      const result = await page.evaluate((tid) => {
        ...
        const getStat = /* @__PURE__ */ __name((key) => {
          const el = document.querySelector(`.status-bar span[title*="${key}"]`);
          return el ? parseInt(el.textContent?.replace(/[^0-9]/g, "") || "0") : 0;
        }, "getStat");
        words = getStat("字");
        views = getStat("阅读");
        comments = getStat("评论");
        const upvoteEl = document.querySelector(`.upvote-num.topic-${tid}`);
        if (upvoteEl) upvotes = parseInt(upvoteEl.textContent?.replace(/[^0-9]/g, "") || "0");
        const tagList = [];
