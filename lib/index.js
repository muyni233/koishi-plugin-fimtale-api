...
function createRenderer(ctx, config, logger, debugLog) {
    ...
    const html = `<!DOCTYPE html><html><head><style>
      ...
      return `<div class="item"><div class="cover-box">${bg}</div><div class="content">
              ...
              <div class="meta-row">
                ...
                <span class="stat" style="color:#e0b141">${ICONS.followers}<span>${r.stats.followers || 0}</span></span>
  ...
  const renderReadPages = /* @__PURE__ */ __name(async (info) => {
    ...
    const html = `<!DOCTYPE html><html><head><style>
        ...
        ${isAlbum ? `#content-scroller { height: auto; width: 100%; display: flex; flex-direction: column; gap: 20px; padding: ${paddingY}px 0; box-sizing: border-box; font-size: ${config.fontSize}px; transform: translateY(0); transition: none; }` : `#content-scroller { height: 100%; width: 100%; column-width: ${contentWidth}px; column-gap: ${columnGap}px; column-fill: auto; padding: ${paddingY}px 0; box-sizing: border-box; font-size: ${config.fontSize}px; line-height: ${lineHeightRatio}; text-align: left; transform: translateX(0); transition: none; }`}
        ...
        ${isAlbum ? `img { max-width: 100%; height: auto; display: block; border-radius: 6px; box-shadow: 0 2px 6px rgba(0,0,0,0.1); margin: 0 auto; }
               .img-box { margin: 0 0 10px 0; }` : `img { max-width: 100%; height: auto; display: block; border-radius: 6px; box-shadow: 0 2px 6px rgba(0,0,0,0.1); }`}
