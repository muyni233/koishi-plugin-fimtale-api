...
export const Config: Schema<Config> = Schema.object({
  ...
  cookies: Schema.string().role('secret').description('浏览器 Cookie (用于解除安全模式，必填)'),
...
export function apply(ctx: Context, config: Config) {
  ...
  const extractImage = (html: string | null | undefined) => {
    if (!html) return null
  ...
  const generateGradient = (str: string) => {
    const hash = crypto.createHash('md5').update(str || 'default').digest('hex')
  ...
  const injectCookies = async (page: Page) => {
  ...
  const scrapeThreadFromHtml = async (threadId: string): Promise<{ valid: boolean, data?: TopicInfo, parent?: TopicInfo, menu?: any[], msg?: string }> => {
    let page: Page
    ...
    try {
      ...
      const result = await page.evaluate((tid) => {
          ...
          // 修复：移除内部辅助函数，直接内联逻辑，防止 __name is not defined 错误
  ...
  const searchThreads = async (keyword: string): Promise<SearchResult[]> => {
    ...
    try {
      ...
      try { await page.waitForSelector('.card', { timeout: 5000 }) } catch {}
      ...
      const results = await page.evaluate(() => {
        ...
        document.querySelectorAll('.card.topic-card').forEach(card => {
          ...
          card.querySelectorAll('.card-action > div span[title]').forEach(s => {
                ...
                if (t.includes('字')) stats.words = v; if (t.includes('阅读')) stats.views = v; if (t.includes('评论')) stats.comments = v
          })
