    ...
    const renderCard = async (info: TopicInfo, parent?: TopicInfo) => {
        ...
        if (config.cardStyle === 'overlay') {
            if (base64Cover) {
                ...
                .author-avatar { width: 26px; height: 26px; border-radius: 50%; border: 1.5px solid rgba(255, 255, 255, 0.2); object-fit: cover; }
                ...
                <div class="card" style="${cardBgStyle}">
                    ...
                    <div class="container">
                        <div>
                            ...
                            <div class="author-row">
                                ${base64Avatar ? `<img class="author-avatar" src="${base64Avatar}"/>` : ''}
                                <span class="author-name">@${info.UserName}</span>
                            </div>
                            ...
                            <div class="summary-box">
                                ${isAlbum && hasImages ? `<div class="album-grid" style="height: 180px; margin-bottom: 6px;">${(base64AlbumImgs as string[]).map(src => `<img src="${src}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 8px;"/>`).join('')}</div><div style="font-size:12px; color:rgba(255,255,255,0.5);">🖼️ 当前章节包含 ${currentImgs.length} 幅图</div>` : `<div class="summary">${summary}</div>`}
                            </div>
                        </div>
